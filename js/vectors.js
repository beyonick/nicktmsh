...
/* --- Роза векторов ------------------------------------------------------------
   ...
   пунктир течёт. На мышь отвечает едва заметно: чуть доворачивается к
   курсору. SVG, а не холст: линия той же толщины, что вся графика
...
function mountCompass(host) {
  ...
  /* Отклик на мышь — минимальный: пока курсор в подвале, роза едва
     доворачивается в его сторону (не больше 6°) и так же медленно
     возвращается. Своё движение стрелок при этом не меняется. */
  const area = host.closest("footer") || document.body;
  let aim = 0;
  let lean = 0;
  if (!reduced) {
    area.addEventListener(
      ...
      (e) => {
        ...
        aim = Math.max(-1, Math.min(1, dx * 2)) * 6;
      },
      { passive: true }
    );
    area.addEventListener("pointerleave", () => (aim = 0));
  }
  ...
  const scene = {
    ...
    draw(t) {
      ...
      arrows.forEach((g, i) => {
        const a = i * 45 + Math.sin(t * 0.9 + i * 1.7) * 22 + t * 9 + lean;
        g.setAttribute("transform", `rotate(${a.toFixed(2)} 100 100)`);
      });
      rings.forEach((c, i) => {
        const dir = i % 2 ? -1 : 1;
        c.setAttribute("transform", `rotate(${(dir * t * (6 + i * 3) + lean * 0.5).toFixed(2)} 100 100)`);
      });
    },
  };
